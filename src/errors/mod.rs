...
impl WowPatcherError {
    pub fn new(category: ErrorCategory, message: impl Into<String>) -> Self {
        Self { category, message: message.into(), cause: None, context: HashMap::new() }
    }
...
mod tests {
    ...
    fn test_error_category_string() {
        assert_eq!(ErrorCategory::FileOperationError.to_string(), "File Operation");
    ...
    fn test_wrap() {
        ...
        let wrapped =
            WowPatcherError::wrap(ErrorCategory::FileOperationError, "wrapped message", base_err);
    ...
    fn test_with_context() {
        ...
        let field = err.get_context("field").and_then(|v| v.downcast_ref::<String>());
        assert_eq!(field, Some(&"username".to_string()));

        let value = err.get_context("value").and_then(|v| v.downcast_ref::<String>());
        assert_eq!(value, Some(&"test123".to_string()));

        let max_length = err.get_context("maxLength").and_then(|v| v.downcast_ref::<usize>());
    ...
    fn test_new_file_error() {
        ...
        let file_path = file_err.get_context("file_path").and_then(|v| v.downcast_ref::<String>());
        assert_eq!(file_path, Some(&"/path/to/file".to_string()));

        let platform = file_err.get_context("platform").and_then(|v| v.downcast_ref::<String>());
    ...
    fn test_new_validation_error() {
        ...
        let field = val_err.get_context("field").and_then(|v| v.downcast_ref::<String>());
        assert_eq!(field, Some(&"age".to_string()));

        let value = val_err.get_context("value").and_then(|v| v.downcast_ref::<i32>());
    ...
    fn test_new_patching_error() {
        ...
        let pattern = patch_err.get_context("pattern").and_then(|v| v.downcast_ref::<String>());
        assert_eq!(pattern, Some(&"portal_pattern".to_string()));

        let suggestion =
            patch_err.get_context("suggestion").and_then(|v| v.downcast_ref::<String>());
    ...
    fn test_new_platform_error() {
        ...
        let operation = plat_err.get_context("operation").and_then(|v| v.downcast_ref::<String>());
        assert_eq!(operation, Some(&"remove_signature".to_string()));

        let platform = plat_err.get_context("platform").and_then(|v| v.downcast_ref::<String>());
        assert_eq!(platform, Some(&std::env::consts::OS.to_string()));
    }
    ...
    fn test_error_chaining() {
        ...
        let level1 = WowPatcherError::wrap(ErrorCategory::FileOperationError, "file error", root_err);
